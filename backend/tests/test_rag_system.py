...
class TestRAGSystemDocumentProcessing:
    ...
    def test_add_course_document_success(self, test_config, tmp_path):
        ...
        test_doc.write_text("""Course Title: Python Fundamentals
Instructor: Jane Doe
...
""")
    ...
    def test_add_course_folder_success(self, test_config, tmp_path):
        ...
        doc1.write_text("""Course Title: Python Basics
...
""")
        ...
        doc2.write_text("""Course Title: JavaScript Fundamentals
Instructor: Jane Doe
...
class TestRAGSystemQuery:
    ...
    def test_query_with_session_management(self, mock_ai_generator_class, test_config):
        ...
        rag.session_manager.get_conversation_history = Mock(return_value="Previous conversation context")
        ...
        rag.session_manager.get_conversation_history.assert_called_once_with("session123")
    ...
    def test_query_with_sources_from_tools(self, mock_ai_generator_class, test_config):
        ...
        # Setup mocks
        mock_ai_generator = Mock()
        mock_ai_generator.generate_response.return_value = "Based on the course materials, Python is versatile."
        mock_ai_generator_class.return_value = mock_ai_generator
        ...
        mock_sources = [
            ...
            {"text": "Python Fundamentals - Lesson 2", "link": "https://example.com/lesson2"}
        ]
    ...
    @patch('rag_system.AIGenerator')
    def test_query_tools_and_tool_manager_passed(self, mock_ai_generator_class, test_config):
...
class TestRAGSystemIntegration:
    ...
    def test_end_to_end_with_real_vector_store(self, test_config, sample_course, sample_course_chunks):
        """Test end-to-end flow with real vector store but mocked AI"""
        with patch('rag_system.AIGenerator') as mock_ai_generator_class:
            # Setup AI mock
            mock_ai_generator = Mock()
            mock_ai_generator.generate_response.return_value = "Python is a programming language for beginners."
            mock_ai_generator_class.return_value = mock_ai_generator
    ...
    def test_real_document_processing_and_query(self, test_config, tmp_path):
        ...
        with patch('rag_system.AIGenerator') as mock_ai_generator_class:
            # Setup AI mock
            mock_ai_generator = Mock()
            mock_ai_generator.generate_response.return_value = "Based on the course content, Python is easy to learn."
            mock_ai_generator_class.return_value = mock_ai_generator
            
            # Create a realistic test document
            test_doc = tmp_path / "python_course.txt"
            test_doc.write_text("""Course Title: Introduction to Python Programming
...
""")
            ...
            assert call_args[1]['tools'] is not None
            assert len(call_args[1]['tools']) >= 1  # At least search tool should be available
